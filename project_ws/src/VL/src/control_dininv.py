...
rospy.init_node("control_invdyn")
...
# qdes = np.array([1.0, 0.3, 1.0, 1.3, 0.2, 1.0])
 
# Configuracion operacional deseada (en metros)
xdes = np.array([0.1,0.3,0.5])
...
while not rospy.is_shutdown():
    ...
    u = M.dot(np.linalg.pinv(J)).dot(ddxdes - dJ.dot(dq) + Kd.dot(de)  + Kp.dot(e)) + b
